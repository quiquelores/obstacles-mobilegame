...
var Game = function(){
  ...
  //console.log("The mouse is at", position.x, position.y);
	...
	this.c.entities.create(Person, {center: {x:30, y:175 },
		update: function(){
	...
	var createObstaclesAndFood = function(game, c){
		...
		if (paused){
			...
			c.entities.create(Obstacle, {center: {x:canvas.width, y: canvas.height*Math.random()}, size:{ x: 30+30*Math.random(), y: 100*Math.random()}});
			//create one food item ~ for every three obstacles
			if(Math.random()*100 < 30){
			c.entities.create(FoodItem, {center: {x:canvas.width, y: canvas.height*Math.random()} });
			}
			setTimeout(function(){ createObstaclesAndFood(game, c);
		}, Math.random()*level());
		}
	}
		...
		this.draw = function(ctx){
			...
			ctx.beginPath();
				...
				ctx.fill();
			this.update = function(timeSinceLastTick){
			if(!paused){this.center.x -= 0.15 * timeSinceLastTick;}}
			if(this.center.x <0){
				this.c.entities.destroy(this);
				}
				};	
